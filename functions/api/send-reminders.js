...
// Runs daily (manual trigger or external scheduler) to send reminder emails for unpaid sessions
...
export async function onRequest(context) {
  try {
    ...
    for (const key of keys) {
      ...
      const shouldSend = sessionData.remindersSent < 3 && (now - sessionData.lastReminderAt) > (oneDayMs - 60 * 60 * 1000);
        ...
        await fetch('https://api.resend.com/emails', {
          ...
          body: JSON.stringify({
            ...
            html: `
              ...
              <p style="background:#fff3cd;padding:15px;border-left:4px solid #ffc107;border-radius:4px;font-size:18px;font-weight:600;">${urgency}</p>
              <p>Your website is still saved and ready to download:</p>
              <p style="text-align:center;margin:30px 0;">
                <a href="${env.SITE_URL}/checkout/${sessionId}" style="display:inline-block;padding:16px 32px;background:#667eea;color:#fff;text-decoration:none;border-radius:8px;font-size:18px;font-weight:600;">Download My Website ($50)</a>
              </p>
              ...
              ${daysRemaining === 1 ? '<p style="background:#f8d7da;padding:15px;border-left:4px solid #dc3545;border-radius:4px;color:#721c24;"><strong>Final Warning:</strong> After today, your website will be permanently deleted and you\'ll need to rebuild it from scratch.</p>' : ''}
              <p>Questions? Just reply to this email!</p>
              <p style="color:#666;font-size:12px;margin-top:30px;">Built with DSGN LABS Web Builder<br><a href="https://web.yourdsgn.pro">web.yourdsgn.pro</a></p>
            `
          })
        });
      }
      ...
      await env.SITE_STORAGE.put(key.name, JSON.stringify(sessionData), {
